...
pub fn list() -> Vec<SessionInfo> {
    let Some(dir) = AppConfig::config_dir().map(|path| path.join("sessions")) else { return Vec::new() };
    let Ok(entries) = fs::read_dir(dir) else { return Vec::new() };
    let mut sessions = entries.filter_map(Result::ok).filter_map(|entry| {
        let path = entry.path();
        if path.extension().and_then(|ext| ext.to_str()) != Some("json") { return None; }
        if path.file_stem().and_then(|stem| stem.to_str()).map(|stem| stem.ends_with(".export")).unwrap_or(false) { return None; }
        let snapshot = load(&path)?;
        let name = path.file_stem()?.to_string_lossy().to_string();
        let modified = entry.metadata().and_then(|meta| meta.modified()).unwrap_or(SystemTime::UNIX_EPOCH);
        Some(SessionInfo { name, path, provider: snapshot.provider, model: snapshot.model, messages: snapshot.messages.len(), modified })
    }).collect::<Vec<_>>();
